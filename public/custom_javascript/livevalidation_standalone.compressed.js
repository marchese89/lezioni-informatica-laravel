...
LiveValidation.prototype = {
	validClass: "LV_valid", invalidClass: "LV_invalid", messageClass: "LV_validation_message", validFieldClass: "LV_valid_field", invalidFieldClass: "LV_invalid_field", initialize: function(D, C) {
		var A = this;
		if (!D) {
		...
		var B = C || {};
		this.validMessage = B.validMessage || '\u2713';//spunta positiva
...
var Validate = {
	Presence: function(B, C) {
		var C = C || {};
		var A = C.failureMessage || "Campo vuoto!";
		...
		var G = E.notANumberMessage || "Deve essere un numero!";
		var H = E.notAnIntegerMessage || "Deve essere un intero!";
		...
		var C = String(C);
		var E = E || {};
		var A = E.failureMessage || "\u2717"; //crocetta
		var B = E.pattern || /./;
		...
		var C = C || {};
		var A = C.failureMessage || "\u2717"; //crocetta
		Validate.Format(B, { failureMessage: A, pattern: /^([a-zA-Z0-9\.]+)@((?:[-a-z0-9]+\.)+[a-z]{2,})$/i });
		return true;
	}, CodiceFiscale: function(B, C) {
		var C = C || {};
		var A = C.failureMessage || "\u2717"; //crocetta
		Validate.Format(B, { failureMessage: A, pattern: /^[a-z]{6}[0-9]{2}[a-z][0-9]{2}[a-z][0-9]{3}[a-z]{1}$/i });
		return true;
	}, Pass: function(B, C) {
		var C = C || {};
		var A = C.failureMessage || "\u2717"; //crocetta
		...
		return true;
	}, SoloTesto: function(B, C) {
		var C = C || {};
		var A = C.failureMessage || "\u2717"; //crocetta
		Validate.Format(B, { failureMessage: A, pattern: /^[a-zA-Z\sèéòàùì'\-\.]*$/i });
		return true;
	}, TestoEnumeri: function(B, C) {
		var C = C || {};
		var A = C.failureMessage || "\u2717"; //crocetta
		Validate.Format(B, { failureMessage: A, pattern: /^[a-zA-Z0-9\sèéòàùì'\-\(\),\+\/\.]*$/i });
		return true;
	}, NumeriConVirgola: function(B, C) {
		var C = C || {};
		var A = C.failureMessage || "\u2717"; //crocetta
		Validate.Format(B, { failureMessage: A, pattern: /^[0-9]*([\.,]{1}[0-9]{2}){0,1}$/i });
		return true;
	}, IBAN: function(B, C) {
		var C = C || {};
		var A = C.failureMessage || "\u2717"; //crocetta
		Validate.Format(B, { failureMessage: A, pattern: /^[A-Z]{2}[0-9]{2}[A-Z]{1}[0-9]{10}[0-9A-Z]{12}$/i });
		return true;
	}, InteriPositivi: function(B, C) {
		var C = C || {};
		var A = C.failureMessage || "\u2717"; //crocetta
		Validate.Format(B, { failureMessage: A, pattern: /^([0-9]){1,}$/i });
		return true;
	}, num_volantini: function(B, C) {
		var C = C || {};
		var A = C.failureMessage || "solo multipli di 1000!"; //crocetta
		Validate.Format(B, { failureMessage: A, pattern: /^([1-9]){1,}000(0){0,}$/i });
		return true;
	}, numTelefono: function(B, C) {
		var C = C || {};
		var A = C.failureMessage || "\u2717"; //crocetta
		Validate.Format(B, { failureMessage: A, pattern: /^[0-9]{6,}$/i });
		return true;
	}, soloNumeri: function(B, C) {
		var C = C || {};
		var A = C.failureMessage || "\u2717"; //crocetta
		Validate.Format(B, { failureMessage: A, pattern: /^[0-9]{1,}$/i });
		return true;
	}
	, StrettMaggiore: function(C, D) {
		if (!D.match) {
		...
		var A = G.wrongLengthMessage || "Ci devono essere almeno " + C + " caratteri!";
		...
		var D = D || {};
		var B = D.failureMessage || "I due valori non corrispondono!";
		var A = D.match.nodeName ? D.match : document.getElementById(D.match);
		if (!A) {
			throw new Error("Validate::Confirmation - There is no reference with name of, or element with id of '" + D.match + "'!");
		}
		...
		var C = C || {};
		var A = C.failureMessage || "Devi accettare!";
		if (!B) {
			Validate.fail(A);
		}
		return true;
